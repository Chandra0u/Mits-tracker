...
<style>
:root {
    --text:#f8fafc; --muted:#b6c2d1; --border:rgba(148,163,184,.22);
}
.stApp {
    background:
...
.hero{position:relative;overflow:hidden;padding:2.2rem 2rem;margin-bottom:1.4rem;
...
.hero:after{content:"";position:absolute;width:260px;height:260px;right:-80px;top:-120px;
 border-radius:50%;background:rgba(56,189,248,.18)}
.badge{display:inline-block;padding:.4rem .8rem;margin-bottom:.9rem;border:1px solid rgba(56,189,248,.35);
 border-radius:999px;background:rgba(56,189,248,.08);color:#7dd3fc;font-size:.8rem;font-weight:800;
 letter-spacing:.08em;text-transform:uppercase}
.hero h1{margin:0;color:var(--text);font-size:clamp(2rem,5vw,4rem);line-height:1.05;letter-spacing:-.045em}
.gradient-text{background:linear-gradient(90deg,#38bdf8,#22d3ee,#a78bfa);
 -webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hero p{max-width:780px;margin:1rem 0 0;color:var(--muted);font-size:1.02rem;line-height:1.75}
div[data-testid="stMetric"]{padding:1rem 1.1rem;border:1px solid var(--border);border-radius:20px;
 background:linear-gradient(145deg,rgba(17,38,64,.94),rgba(12,27,47,.92));box-shadow:0 14px 30px rgba(0,0,0,.18)}
div[data-testid="stMetricLabel"]{color:var(--muted)}
div[data-testid="stMetricValue"]{color:var(--text)}
.stButton>button,.stDownloadButton>button{min-height:3rem;border:0;border-radius:14px;
 background:linear-gradient(90deg,#0284c7,#06b6d4);color:#fff;font-weight:800;
 box-shadow:0 10px 25px rgba(6,182,212,.2);transition:.2s}
.stButton>button:hover,.stDownloadButton>button:hover{transform:translateY(-1px);color:#fff;
 box-shadow:0 14px 30px rgba(6,182,212,.3)}
div[data-baseweb="input"]>div,div[data-baseweb="select"]>div,div[data-baseweb="base-input"]>div{
 border-radius:13px!important;border-color:var(--border)!important;background:rgba(8,22,39,.92)!important}
div[data-testid="stDataFrame"]{overflow:hidden;border:1px solid var(--border);border-radius:18px}
.footer-card{margin-top:2rem;padding:1.35rem;text-align:center;border:1px solid var(--border);
 border-radius:22px;background:rgba(9,24,42,.84);color:var(--muted)}
.footer-card strong{color:var(--text)} .footer-card a{color:#67e8f9;font-weight:700;text-decoration:none}
@media(max-width:700px){.block-container{padding-left:.8rem;padding-right:.8rem}.hero{padding:1.6rem 1.2rem;border-radius:22px}}
</style>
""",
...
async def scrape_attendance_async(roll, password):
    async with async_playwright() as p:
        ...
        try:
            ...
            # Streamlit Community Cloud installs Chromium from packages.txt.
...
def scrape_attendance(roll, password):
    ...
    try:
        ...
        env = os.environ.copy()
        # Do not force /ms-playwright; use system Chromium from packages.txt.
        ...
        if not proc.stdout.strip():
            raise RuntimeError("The scraper returned an empty response.")
...
st.session_state.setdefault("attendance_data", None)
st.session_state.setdefault("last_roll", "")
st.session_state.setdefault("show_overall_calc", False)
...
<section class="hero">
  ...
  <p>A modern attendance dashboard created by <strong>Chandra Sekhar</strong>.
  View MITS IMS attendance, compare subjects and plan future classes from one clean,
  mobile-friendly dashboard.</p>
</section>
""",
    unsafe_allow_html=True,
)
...
if st.session_state.attendance_data:
    ...
    df["Calculated_Percentage"] = (df["Attended"] / df["Conducted"].replace(0, pd.NA) * 100).fillna(0).astype(float)
    ...
    m1.metric("Overall Attendance", f"{overall_attendance:.1f}%")
    m2.metric("Total Attended", total_attended)
    m3.metric("Total Conducted", total_conducted)
    m4.metric("Subjects", len(df))
    ...
    with fc2:
        selected_subjects = st.multiselect("Subjects", df["Subject"].tolist(), default=df["Subject"].tolist())
    with fc3:
    ...
    st.dataframe(display_df.style.map(color_percentage, subset=["Percentage"]), use_container_width=True, hide_index=True)
    ...
    if filtered_df.empty:
        st.info("Select at least one subject to display the chart.")
    else:
        st.bar_chart(filtered_df[["Subject", "Calculated_Percentage"]].set_index("Subject"), use_container_width=True)
...
<div class="footer-card">
  ...
  <a href="https://www.linkedin.com/in/chandra-sekhar-talari-38040832b" target="_blank" rel="noopener noreferrer">LinkedIn</a>
  &nbsp; • &nbsp;
  <a href="https://www.instagram.com/urs_sekhar18" target="_blank" rel="noopener noreferrer">Instagram</a>
